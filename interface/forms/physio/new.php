...
if ($formid) {
    $sql = "SELECT * FROM `form_physio` WHERE id=? AND pid = ? AND encounter = ?";
...
<html>
    <head>
        <title><?php echo xlt("Physio Form"); ?></title>
        ...
        <script type="text/javascript">
            ...
            function removeVal(rowid)
            {
                ...
                document.getElementById("physio_type_" + rowid1[1]).value = '';
            }
        ...
        <div class="container">
            <div class="row">
                <div class="page-header">
                    <h2><?php echo xlt('Physio Form'); ?></h2>
                </div>
            </div>
            <div class="row">
            <?php echo "<form method='post' name='my_form' " . "action='$rootdir/forms/physio/save.php?id=" . attr($formid) . "'>\n"; ?>
                <fieldset>
                    ...
                    <div class="form-group">
                        ...
                        <div class="forms col-xs-2">
                            <label for="physio_type_<?php echo attr($key) + 1; ?>" class="h5"><?php echo xlt('Type'); ?>:</label>
                            <select name="physio_type[]" id="physio_type_<?php echo attr($key) + 1; ?>" class="form-control physio_type">
                                <option value=""></option>
                                <?php foreach ($physio_type as $value) :
                                    $selected = ($value['value'] == $obj{"physio_type"}) ? 'selected="selected"' : '';
                                    ?>
                    ...
                    <div class="tb_row" id="tb_row_1">
                        <div class="form-group">
                            ...
                            <div class="forms col-xs-2">
                                <label for="physio_type_1" class="h5"><?php echo xlt('Type'); ?>:</label>
                                <select name="physio_type[]" id="physio_type_1" class="form-control physio_type">
                                    <option value=""></option>
                                    <?php foreach ($physio_type as $value) :
                                        $selected = ($value['value'] == $obj{"physio_type"}) ? 'selected="selected"' : '';
                                        ?>
