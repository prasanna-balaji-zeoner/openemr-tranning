...
$count = $_POST["count"];
$physio_type = $_POST['physio_type'];

if ($id && $id != 0) {
    sqlStatement("DELETE FROM `form_physio` WHERE id=? AND pid = ? AND encounter = ?", array($id, $_SESSION["pid"], $_SESSION["encounter"]));
    $newid = $id;
} else {
    $res2 = sqlStatement("SELECT MAX(id) as largestId FROM `form_physio`");
    ...
    addForm($encounter, "Physio Form", $newid, "physio", $_SESSION["pid"], $userauthorized);
}
...
if (!empty($count)) {
    foreach ($count as $key => $codeval) :
        ...
        $physio_type_val = $physio_type[$key] ? $physio_type[$key] : 'NULL';
        $sets = "id    = ". add_escape_custom($newid) .",
            ...
            physio_type = '" .add_escape_custom($physio_type_val). "'";
        sqlInsert("INSERT INTO form_physio SET $sets");
    endforeach;
}
